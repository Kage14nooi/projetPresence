...
const ParcoursList: React.FC<ParcoursListProps> = ({
  ...
  const filtered = parcours.filter((p) =>
    p.parcours_nom?.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 p-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-white mb-1">Parcours</h1>
                <p className="text-blue-100">
                  {filtered.length} parcours{" "}
                  {searchTerm && `sur ${parcoursCount} au total`}
                </p>
              </div>
            </div>
