...
const SeanceForm: React.FC<SeanceFormProps> = ({
  ...
  const handleLocalSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.matiere_id) newErrors.matiere_id = "La matière est requise";
    if (!formData.appareil_id) newErrors.appareil_id = "L'appareil est requis";
    if (!formData.date_seance) newErrors.date_seance = "La date est requise";
    ...
    <form onSubmit={handleLocalSubmit} className="p-6 space-y-4">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            setFormData({ ...formData, appareil_id: Number(e.target.value) })
          }
          className={`w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 ${
            errors?.appareil_id ? "border-red-500" : "border-gray-300"
          }`}
        >
          <option value="">-- Sélectionner un appareil --</option>
          {Array.isArray(appareils) && appareils.length > 0 ? (
            appareils.map((appareil) => (
              <option key={appareil.appareil_id} value={appareil.appareil_id}>
                {appareil.appareil_nom}
              </option>
            ))
          ) : (
        ...
        {errors?.appareil_id && (
          <p className="text-red-500 text-sm mt-1">{errors.appareil_id}</p>
        )}
      </div>
