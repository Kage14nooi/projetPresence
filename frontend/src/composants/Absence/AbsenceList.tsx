...
const AbsenceList: React.FC<AbsenceListProps> = ({ absences = [] }) => {
  ...
  const seanceOptions = Array.from(
    new Set(
      absences
        .filter(
          (a) =>
            ...
            (!selectedNiveau || a.etudiant.niveau_id === selectedNiveau) &&
            (!selectedMatiere || a.seance.matiere_id === selectedMatiere) &&
            (!selectedDate || a.seance.date_seance === selectedDate)
        )
        .map(
          (a) =>
            `${a.seance.matiere_id}_${a.seance.date_seance}_${a.seance.heure_debut}`
        )
    )
  );
  ...
  const filteredAbsences = absences
    ...
    .filter((a) => !selectedDate || a.seance.date_seance === selectedDate)
    .filter((a) => {
      if (!selectedSeance) return true;
      const val = `${a.seance.matiere_id}_${a.seance.date_seance}_${a.seance.heure_debut}`;
      return val === selectedSeance;
    })
    .filter((a) => {
      if (!searchTerm) return true;
      const e = a.etudiant;
  ...
  return (
    <div className="flex flex-col min-h-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                >
                  ...
                  {seanceOptions.map((s) => {
                    const [matiere_id, date, heure] = s.split("_");
                    const matiereNom = absences.find(
                      (a) =>
                        a.seance.matiere_id.toString() === matiere_id &&
                        a.seance.date_seance === date &&
                        a.seance.heure_debut === heure
                    )?.seance.matiere.matiere_nom;
                    return (
                      <option key={s} value={s}>
                        {matiereNom} - {date} • {heure}
                      </option>
                    );
                  })}
                </select>
      ...
      {/* Table et pagination */}
      {/* ... Le reste de ton code pour la table et pagination reste identique ... */}
      {/* Table avec style unifié */}
      <div className="flex-1 bg-white overflow-hidden">
        <div className="overflow-x-auto flex-1">
          <table className="w-full">
